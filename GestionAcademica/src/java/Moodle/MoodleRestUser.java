...
public class MoodleRestUser implements Serializable {
    ...
    public static MoodleUser[] getUsersById(Long[] userids) throws MoodleRestUserException, UnsupportedEncodingException, MoodleRestException {
      ...
      //  boolean processed=false;
        String functionCall=MoodleCallRestWebService.isLegacy()?MoodleServices.MOODLE_USER_GET_USERS_BY_ID.toString():MoodleServices.CORE_USER_GET_USERS_BY_FIELD.toString();
        try {
            StringBuilder data=new StringBuilder();
            if (MoodleCallRestWebService.getAuth()==null)
                throw new MoodleRestUserException();
    ...
    public MoodleUser[] __getUsersById(String url, String token, Long[] userids) throws MoodleRestUserException, UnsupportedEncodingException, MoodleRestException {
        ...
        try {
            ...
            UserEnrolledCourse enrolledCourse=null;
            for (int i=0;i<userids.length;i++) {
                if (userids[i]<1) throw new MoodleRestUserException(MoodleRestUserException.INVALID_USERID);                 
                data.append("&").append(URLEncoder.encode("field", MoodleServices.ENCODING.toString())).append("=").append("id");
                data.append("&").append(URLEncoder.encode("values["+i+"]", MoodleServices.ENCODING.toString())).append("=").append(userids[i]);
            }
